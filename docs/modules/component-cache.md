# 缓存组件（component-cache）— Contract 轨
...
缓存组件（`component-cache`）是基于 [Caffeine](https://github.com/ben-manes/caffeine) 的高性能本地缓存组件，提供完整的缓存读写、TTL 过期管理和 List 类型缓存支持。
...
**模块坐标：** `org.smm.archetype:component-cache`
...
classDiagram
    ...
    AbstractCacheComponent <|-- CaffeineCacheComponent : extends
    CaffeineCacheComponent --> CacheValueWrapper : 内部类
    CaffeineCacheComponent --> CaffeineExpiry : 内部类
    CacheAutoConfiguration ..> CacheProperties : @EnableConfigurationProperties
    CacheAutoConfiguration ..> CaffeineCacheComponent : @Bean
```
...
| 类名 | 职责 | 关键方法 |
|------|------|----------|
| `CacheComponent` | 缓存操作接口，定义 10 个缓存方法 | `get`, `put`, `delete`, `hasKey`, `expire` 等 |
| `AbstractCacheComponent` | 抽象基类，Template Method 模式骨架 | `final` 公开方法 + `do*` 扩展点 |
| `CaffeineCacheComponent` | Caffeine 实现，每个 Entry 独立过期 | `CacheValueWrapper` 包装值+过期时间 |
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `component.cache.enabled` | `boolean` | `true` | 是否启用缓存组件 |
| `component.cache.initial-capacity` | `Integer` | `1000` | Caffeine 缓存初始容量 |
| `component.cache.maximum-size` | `Long` | `10000` | Caffeine 缓存最大条目数 |
| `component.cache.expire-after-write` | `Duration` | `30d` | 写入后默认过期时间 |
| `component.cache.expire-after-access` | `Duration` | `30d` | 访问后过期时间（配置项存在，当前未使用） |

> **注意**：`expire-after-access` 配置项在 `CacheProperties` 中定义，但当前 `CaffeineCacheComponent` 使用自定义 `CaffeineExpiry` 策略实现每个 Entry 独立过期，未使用此配置项。
...
public class ConfigService {
    ...
    private final CacheComponent cacheClient;
...
`component-cache` 作为底层存储，为限流（`app/.../shared/aspect/ratelimit`）和幂等（`app/.../shared/aspect/idempotent`）模块提供缓存支持。这些模块在内部注入 `CacheComponent` 实现令牌桶状态和幂等 Key 的本地存储：
...
public BaseResult<List<Data>> getData(DataRequest request) {
    // 限流逻辑由 Bucket4j + CacheComponent 自动处理
...
### 上游依赖
- [docs/architecture/design-patterns.md](../architecture/design-patterns.md) — Template Method 模式详细说明（AbstractCacheComponent 骨架实现）

### 下游消费者
- `app/.../shared/aspect/ratelimit/` — 限流模块，使用 CacheComponent 存储令牌桶状态
- `app/.../shared/aspect/idempotent/` — 幂等模块，使用 CacheComponent 存储幂等 Key
...
> 作为脚手架项目的通用基础设施，Caffeine 的零运维成本和纳秒级延迟更适合快速开发场景。如需分布式缓存，可参考 [architecture/design-patterns.md](../architecture/design-patterns.md) 中的 Template Method 模式扩展点，实现基于 Redis 的 `CacheComponent` 子类。

## 变更历史
| 日期 | 变更内容 |
|------|---------|
| 2026-04-15 | 补充下游消费者引用（IdempotentAspect 使用 CacheComponent 作为幂等 Key 存储后端） |
| 2025-04-14 | 初始创建 |
