...
class AuthAutoConfigurationUTest {
    ...
    class DefaultConfigTest {
        ...
        @Test
        @DisplayName("默认应注册 AuthComponent Bean")
        void shouldRegisterAuthComponentByDefault() {
            contextRunner
                    .run(context -> {
    ...
    class DisabledTest {
        ...
        @Test
        @DisplayName("enabled=false 时不应注册 AuthComponent Bean")
        void shouldNotRegisterAuthComponentWhenDisabled() {
            contextRunner
                    .withPropertyValues("component.auth.enabled=false")
                    .run(context -> {
                        assertThat(context).doesNotHaveBean(AuthComponent.class);
                    });
        }
    }
    ...
    class EnabledTest {
        ...
        @Test
        @DisplayName("enabled=true 时应注册 AuthComponent Bean")
        void shouldRegisterAuthComponentWhenEnabled() {
            contextRunner
    ...
    class PropertiesBindingTest {
        ...
        void excludePathsShouldBind() {
            contextRunner
                    .withPropertyValues(
                            "component.auth.exclude-paths=/api/test/**,/api/auth/**"
                    )
                    .run(context -> {
                        AuthProperties properties = context.getBean(AuthProperties.class);
