...
class OtelIntegrationITest extends IntegrationTestBase {
    ...
    class ErrorAndBoundaryPathTest {
        ...
        @Test
        @DisplayName("MFT: 异常请求也携带 OTel traceId")
        void should_exceptionResponseContainTraceId() {
            // when: 调用抛出 BizException 的端点
            webTestClient.get().uri("/api/test/exception")
                    ...
                    .consumeWith(response -> {
                        ...
                        // then: 错误响应仍包含有效的 32 字符 hex traceId
                        String traceId = (String) body.get("traceId");
                        assertThat(traceId)
                                .as("异常响应的 BaseResult.traceId 应为有效的 32 字符 hex")
                                .isNotNull()
                                .hasSize(32)
                                .matches("[0-9a-f]{32}");
        ...
        @Test
        @DisplayName("INV: 不同端点产生不同 traceId（请求隔离）")
        void should_differentRequestsProduceDifferentTraceIds() {
            // 第一次请求
            Map<String, Object> firstResponse = webTestClient.get().uri("/api/test/hello")
                    .exchange()
                    .expectStatus().isOk()
            ...
            // 第二次请求
            Map<String, Object> secondResponse = webTestClient.get().uri("/api/test/hello")
                    .exchange()
                    .expectStatus().isOk()
                    .returnResult(Map.class)
                    .getResponseBody()
            ...
            String firstTraceId = (String) firstResponse.get("traceId");
            String secondTraceId = (String) secondResponse.get("traceId");

            assertThat(firstTraceId)
                    .as("第一次请求应有有效的 traceId")
                    .isNotNull()
                    .hasSize(32);

            assertThat(secondTraceId)
                    .as("第二次请求应有有效的 traceId")
                    .isNotNull()
                    .hasSize(32);

            assertThat(firstTraceId)
                    .as("不同请求应产生不同的 traceId（请求隔离）")
                    .isNotEqualTo(secondTraceId);
        }

        @Test
        @DisplayName("DIR: 404 错误响应不包含 traceId（Spring Boot 默认行为）")
        void should_404NotContainTraceId() {
            // when: 调用不存在的端点
            webTestClient.get().uri("/api/test/nonexistent-endpoint-xyz")
                    .exchange()
                    .expectStatus().is4xxClientError();
            // 注意：404 由 Spring Boot 默认错误处理返回，
            // 不会经过 OTel BaseResult 封装，因此响应格式不同于正常 API 响应
        }
    }
