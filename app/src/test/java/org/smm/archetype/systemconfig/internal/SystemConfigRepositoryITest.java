...
class SystemConfigRepositoryITest extends IntegrationTestBase {
    ...
    class Save {
        ...
        void should_insertNewConfig_withGeneratedId() {
            ...
            newConfig.setDisplayName(org.smm.archetype.systemconfig.internal.DisplayName.of("测试配置"));
            ...
            // 清理：物理删除测试数据，避免影响其他 ITest
            jdbcTemplate.update("DELETE FROM system_config WHERE config_key = 'test.new.key'");
        }
        ...
        void should_updateExistingConfig() {
            ...
            // 清理：恢复原始值，避免影响其他 ITest
            jdbcTemplate.update("UPDATE system_config SET config_value = ? WHERE config_key = 'site.description'",
                    originalValue);
        }
    }
}
